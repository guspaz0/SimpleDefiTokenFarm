...
describe('TokenFarm', function () {
	...
	async function deployTokenFarmFixture() {
		...
		const encodeFunctionInitialize = await web3.eth.abi.encodeFunctionCall(
		...
		const proxyAdminContract = new web3.eth.Contract(ProxyAdminArtifact.abi);
		const rawProxyAdmin = proxyAdminContract.deploy({
			data: ProxyAdminArtifact.bytecode,
			arguments: [Proxy.options.address]
		})
		const estimateGasProxyAdmin = await rawProxyAdmin.estimateGas({from: deployer});
		const ProxyAdmin = await rawProxyAdmin.send({
			from: deployer,
			gas: estimateGasProxyAdmin.toString(),
			gasPrice: '10000000000'
		});
	...
	this.beforeAll(async function(){
		const {	TokenFarm, deployer, DappToken, LpToken, Proxy} = await loadFixture(deployTokenFarmFixture)
	...
	describe("Que un usuario pueda deshacer el staking de todos los tokens LP depositados y reclamar recompensas pendientes, si las hay.", function(){
		it("when user call withdraw method, his TokenFarms balance must be 0",async function(){
			...
			const withdraw = await proxy.methods.withdraw().send({from: user.address})
			console.log(withdraw.events.Withdraw.returnValues)
			const userinfo = await proxy.methods.getUserInfo(user.address).call({from: user.address});
	...
	describe("The owner can be able to change the reward per static range of blocks", function(){
		it("when the owner call the modifier method, it should be saved", async function(){
			...
			const updatedReward = await proxy.methods.updateRewardRange(10,1e15).send({from: owner})
