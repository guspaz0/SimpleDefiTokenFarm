...
describe('TokenFarmV2', function () {
	...
	async function deployTokenFarmFixture() {
		...
		const rawContract = TokenFarmContract.deploy({
			...
			arguments: [
				...
				fee, //fee
				RewardPerBlock[10], //reward per block in range 10
				RewardPerBlock[100], //reward per block in range 100
				RewardPerBlock[1000]  //reward per block in range 1000
			],
		});
	...
	describe("El Owner de TokenFarm puede retirar los fee obtenidos por las recompensas de los usuarios", function() {
		it("the fee tax must be a number greater than 0 and less than 100",async function(){
			const [owner] = await web3.eth.getAccounts();
			const fee = await tokenFarm.methods.fee().call({from: owner});
		...
		it("after the owner call withdrawFee, it should receive the amount in DappTokens", async function() {
			const owner = await tokenFarm.methods.owner().call();
